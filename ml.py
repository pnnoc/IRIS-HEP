...
def load_models():
    ...
    model_even.load_model(config["ml"]["XGBOOST_MODEL_PATH_EVEN"]) # model_even.load_model('models/model_even.json')

    global model_odd
    model_odd = XGBClassifier()
    model_odd.load_model(config["ml"]["XGBOOST_MODEL_PATH_ODD"]) # model_odd.load_model('models/model_odd.json')
